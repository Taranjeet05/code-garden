    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center h-screen bg-gradient-to-br from-blue-50 to-blue-100">
                ...
                <span className="mt-4 text-blue-700 font-medium animate-pulse">Loading users...</span>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 py-10 px-4">
            <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg p-8">
                <h1 className="text-3xl font-extrabold text-blue-700 mb-6 flex items-center gap-2">
                    <span>All User List</span> <span role="img" aria-label="note">📝</span>
                </h1>

                {!user || user.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16">
                        <p className="mb-4 text-gray-500">No users found. Click below to fetch users.</p>
                        <button
                            onClick={fetchUser}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {user.map((u) => (
                            ...
                            >
                                <h2 className="font-bold text-lg text-blue-800 group-hover:text-blue-900 transition">{u.name}</h2>
