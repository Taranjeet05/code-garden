...
function decrypt(encryptedText, n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const oddHalf = encryptedText.slice(0, half);
    const evenHalf = encryptedText.slice(half);
    let result = "";

    for (let j = 0; j < encryptedText.length; j++) {
      if (j % 2 === 0) {
        result += evenHalf[Math.floor(j / 2)] || "";
      } else {
        result += oddHalf[Math.floor(j / 2)] || "";
      }
    }
...
console.log(encrypt("012345", 6));
console.log(decrypt("012345", 6));
